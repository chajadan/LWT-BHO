...
public:
	...
	*/
	...
	{
		out.append(L"<sup>");
		AppendWithSegues(out, wWordCount, pRec->wStatus, pRec);
		out.append(L"</sup>");
	}
	...
	void AppendInfoBoxJavascript(IHTMLDocument2* pDoc, IHTMLElement* pBody)
	{
		...
		out.append(
			...
			L"{"
			//L"	if (e.srcElement.id == 'lwtinlinestat')"
			L"	fixPageXY(e);"
			...
			L"		lwtcontextexit();"
			L"	}"
			//L"	document.getElementById('lwtinfobox').style.display = 'none';"
			L"}"
...
L"var x = getMWList(e.srcElement);"
L"if (x[0] != '') alert(x[0]);"
L"if (x[1] != '') alert(x[1]);"
L"if (x[2] != '') alert(x[2]);"
L"if (x[3] != '') alert(x[3]);"
...
L"			return mwList;"
L""
L"		if (elem.getAttribute('lwtsent') == 'lwtsent')"
L"			return mwList;"
L""
